...
three frames. Background geometry is evaluated against a separate 1\,cm
...
Dynamic~\cite{palazzolo2019bonn} contributes three short box-manipulation
...
class. The state is counted as represented when $r(G,M)\ge0.5$. A moved object
...
recorded motion within A and static reference annotation disagree; we retain the
reported score rather than correcting the annotation after evaluation. Relative
...
Restricting evaluation to the observed domain leaves a reported gap of
2.6--3.3 percentage points, so reference incompleteness does not fully explain
the geometry loss. The available records do not contain the complete
per-method observed-region table; these aggregate gaps are not a substitute for
one.
